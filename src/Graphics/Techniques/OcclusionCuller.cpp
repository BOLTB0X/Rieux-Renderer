...
void OcclusionCuller::DispatchPhase1(const DispatchPhase1Params& param) {
    ...
    // 카운터 초기화를 위해 Copy Dest로 상태 변경 (Final & Phase1Culled 모두)
    D3D12_RESOURCE_BARRIER toCopyDest[4] = {
        ...
        CD3DX12_RESOURCE_BARRIER::Transition(m_phase1CulledMainCounterBuffer.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST),
        CD3DX12_RESOURCE_BARRIER::Transition(m_phase1CulledVaseCounterBuffer.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST)
    };
    ...
    D3D12_RESOURCE_BARRIER toUAV[8] = {
        ...
        CD3DX12_RESOURCE_BARRIER::Transition(m_phase1CulledMainCommandsBuffer.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_UNORDERED_ACCESS),
        CD3DX12_RESOURCE_BARRIER::Transition(m_phase1CulledVaseCommandsBuffer.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_UNORDERED_ACCESS)
    };
    ...
    uint32_t isPhase2 = 0;
    cmdList->SetComputeRoot32BitConstants(RendererState::OcclusionPhaseIndex, 1, &isPhase2, 0);
    ...
    D3D12_RESOURCE_BARRIER toPhase1End[8] = {
        // 1~4: Final 버퍼들은 다음 패스(Depth/Sponza)에서 Indirect Argument로 사용하기 위해 전환
        ...
        // 5~8: Phase 1 Culled 버퍼들(Command & Counter)은 Phase 2에서 SRV 위해 NON_PIXEL_SHADER_RESOURCE로 전환
...
void OcclusionCuller::DispatchPhase2(const DispatchPhase2Params& param) {
    ...
    // Phase 1에서 그렸던 Final 버퍼를 다시 Append하기 위해 UAV로 전환 (Vase 포함)
    ...
    // t1~t4: Frustum 데이터 대신 Phase 1에서 실패한(Culled) SRV를 입력으로 사용
    ...
    // u0~u3: 최종 통과 버퍼 (여기에 계속 Append 됨)
    cmdList->SetComputeRootDescriptorTable(RendererState::OcclusionFinalMainCommandsIndex, m_heapAllocator->GetGPUHandle(m_finalMainVisibleDescIndex));
    cmdList->SetComputeRootDescriptorTable(RendererState::OcclusionFinalVaseCommandsIndex, m_heapAllocator->GetGPUHandle(m_finalVaseVisibleDescIndex));
    cmdList->SetComputeRootDescriptorTable(RendererState::OcclusionFinalMainCountIndex, m_heapAllocator->GetGPUHandle(m_finalMainCounterDescIndex));
    cmdList->SetComputeRootDescriptorTable(RendererState::OcclusionFinalVaseCountIndex, m_heapAllocator->GetGPUHandle(m_finalVaseCounterDescIndex));

    // u4~u7: Phase 2에서는 실패작(Culled)을 기록하지 않으므로 사용 안 함.
    // 단, D3D12 유효성 검사 경고를 피하기 위해 이미 바인딩된 Final UAV를 더미(Dummy)로 채워줌
    cmdList->SetComputeRootDescriptorTable(RendererState::OcclusionCulledMainCommandsIndex, m_heapAllocator->GetGPUHandle(m_finalMainVisibleDescIndex));
    cmdList->SetComputeRootDescriptorTable(RendererState::OcclusionCulledVaseCommandsIndex, m_heapAllocator->GetGPUHandle(m_finalVaseVisibleDescIndex));
    cmdList->SetComputeRootDescriptorTable(RendererState::OcclusionCulledMainCountIndex, m_heapAllocator->GetGPUHandle(m_finalMainCounterDescIndex));
    cmdList->SetComputeRootDescriptorTable(RendererState::OcclusionCulledVaseCountIndex, m_heapAllocator->GetGPUHandle(m_finalVaseCounterDescIndex));
...
void OcclusionCuller::BuildBuffers(ID3D12Device* device) {
    ...
    BuildGroupResources(device, m_maxMainCount, m_phase1CulledMainCommandsBuffer, m_phase1CulledMainUAVIndex, m_phase1CulledMainCounterBuffer, m_phase1CulledMainCounterUAVIndex);
    BuildGroupResources(device, m_maxVaseCount, m_phase1CulledVaseCommandsBuffer, m_phase1CulledVaseUAVIndex, m_phase1CulledVaseCounterBuffer, m_phase1CulledVaseCounterUAVIndex);
